...
order, ignoring duplicates), the same hypotheses (in the same order, including
potential duplicates), and the same conclusion.

Because of the canonical representation of an InferenceRule, the implementation
of Eq InferenceRule is as simple as "deriving Eq" (as is done above).
